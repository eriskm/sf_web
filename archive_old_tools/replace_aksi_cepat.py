...
# Define the new HTML for Aksi Cepat
new_aksi_cepat = """<div style="display: flex; gap: 12px; flex-wrap: wrap;">
                        <!-- Service Baru -->
...
pattern = re.compile(r'<div style="display: flex; gap: 8px; flex-wrap: wrap;">\s*<a href="/tambah".*?</div>\s*</div>\s*</div>', re.DOTALL)
match = pattern.search(html)

if match:
    html = html[:match.start()] + new_aksi_cepat + '\n                </div>' + html[match.end():]
    with open('templates/dashboard.html', 'w', encoding='utf-8') as f:
        f.write(html)
    print("Replaced Aksi Cepat successfully!")
else:
